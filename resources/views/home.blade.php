...
<body>
    <main class="bg-slate-900">
        <section class="h-[100vh] flex justify-center items-center">
            <section class="w-full max-w-[400px] p-3">
                ...
                <form action="{{ route('save-image') }}" method="post" enctype="multipart/form-data">
                    ...
                    <input type="text" name="title" id="title" placeholder="Name image" class="block w-full p-2 mb-10 bg-transparent border-indigo-900 border-b-2 outline-0 focus:border-sky-800">
        ...
        <section class="p-4 max-w-[960px] mx-auto grid grid-cols-1 gap-4 justify-items-center sm:grid-cols-2 lg:grid-cols-3">
            @for ($i = 0; $i < 9; $i++)
                <div class="bg-red-500 w-[300px] h-[300px] sm:even:justify-self-start sm:odd:justify-self-end lg:even:justify-self-auto lg:odd:justify-self-auto flex justify-center items-center text-white last:bg-sky-500">{{ $i + 1 }}</div>
            @endfor
        </section>
    </main>
    <script>
